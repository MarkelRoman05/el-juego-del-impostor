...
(async () => {
  ...
  await lu2P;
  const startedP = [onEvent(host, 'round:started'), ...sockets.slice(1).map(s => onEvent(s, 'round:started'))];
  ...
  if (ack.ok) {
    ...
    console.log('roles:', roles.map(r => ({ role: r.role, word: r.word, cat: r.category })));
  }
  process.exit(0);
